...
def convert_predicted_logits_to_segmentation_with_correct_shape_torch(
    predicted_logits: Union[torch.Tensor, np.ndarray],
    plans_manager: PlansManager,
    configuration_manager: ConfigurationManager,
    label_manager: LabelManager,
    ...
    if not return_probabilities:
        ...
        predicted_probabilities = label_manager.apply_inference_nonlin(predicted_logits)
        # segmentation = label_manager.convert_probabilities_to_segmentation(predicted_probabilities)
        assert label_manager.has_regions == False
        segmentation = predicted_probabilities.argmax(0)
    del predicted_logits

    # put segmentation in bbox (revert cropping)
    segmentation_reverted_cropping = torch.zeros(
        properties_dict["shape_before_cropping"],
    ...
    if return_probabilities:
        ...
        #     properties_dict["shape_before_cropping"],
        # )
        probs_reverted_cropping = torch.zeros(
            (predicted_probabilities.shape[0], *properties_dict["shape_before_cropping"]),
        ...
        predicted_probabilities = insert_crop_into_image(
            probs_reverted_cropping, predicted_probabilities, properties_dict["bbox_used_for_cropping"]
        )
        # predicted_probabilities = predicted_probabilities.cpu().numpy()
        # revert transpose
        predicted_probabilities = predicted_probabilities.permute(
            [0] + [i + 1 for i in plans_manager.transpose_backward]
        )
        torch.set_num_threads(old_threads)
        return segmentation_reverted_cropping, predicted_probabilities
...
def export_prediction_from_logits(
    ...
    rw.write_seg(
        segmentation_final, output_file_truncated + dataset_json_dict_or_file["file_ending"], properties_dict
    )
